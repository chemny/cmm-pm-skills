...
> **Boundary:** This skill supplies the PRD structure; it is not the complete Stage ④ workflow. A full PRD must be orchestrated by `commands/write-prd.md`, inherit upstream research gates, run incremental Deep Research for new/expired load-bearing facts, and must not claim research completion from this template alone.
