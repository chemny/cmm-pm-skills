...
作为统一入口，先判断用户要完整流程、单项直达，还是继续已有项目。必须读取并遵循内置的 [`CONVENTIONS.md`](references/CONVENTIONS.md)。
...
- 市场可行性、MRD、完整市场研究 → 完整执行 [`market-analysis`](references/commands/market-analysis.md)；其中再调用 `competitor-analysis`、`market-sizing` 等方法。
- 需求洞察 → 完整执行 [`insight`](references/commands/insight.md)。
- 产品战略 → 完整执行 [`strategy`](references/commands/strategy.md)。
- PRD → 完整执行 [`write-prd`](references/commands/write-prd.md)；其中再调用 `create-prd` 模板。
- 界面与原型 → 完整执行 [`wireframe`](references/commands/wireframe.md)。
- 用户故事 → 完整执行 [`write-stories`](references/commands/write-stories.md)。
- 技术方案、模型/API/供应商选型 → 完整执行 [`tech-design`](references/commands/tech-design.md)。
- 测试 → 完整执行 [`test-scenarios`](references/commands/test-scenarios.md)。
- 上线 → 完整执行 [`plan-launch`](references/commands/plan-launch.md)。
- 数据迭代 → 完整执行 [`analyze-cohorts`](references/commands/analyze-cohorts.md)。

阶段命令已内置在 `references/commands/`，无论宿主按完整插件安装，还是只安装 Skills，都必须从该目录读取完整阶段规则。不得降级为只跑叶子 Skill 后声称阶段完成。
