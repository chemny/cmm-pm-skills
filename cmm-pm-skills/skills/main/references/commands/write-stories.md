...
- **Don't fabricate effort estimates** — sizing is the team's call. Give relative size as *indicative only*, and mark any story resting on an unsolved approach as `?(spike-gated)`, never a fake S/M/L.
- **A story that is critical but whose implementation approach is undefined → make it an EPIC with a prerequisite SPIKE** that resolves the approach AND defines the measurable acceptance protocol; don't pretend it's one estimable story.
- **No vague AC.** "works well / 体验好 / 更像你" are not testable. Every AC needs a number, a protocol, or a binary check QA can run without interpretation.
- Cover the **foundational/infra stories the PRD implies** (background service, account/auth, error handling) — not just the happy-path features.
