...
Tests are derived from the docs, so the docs come first. If the project-relative `documentation/*.md` set is missing or thin, run `/document-app` (and `/derive-tests` reads `flows.md`, `permissions.md`, and `automation.md` most heavily). You cannot map coverage to rules you never wrote down — where intent is absent, say so rather than inventing rules to test.
...
Optionally write the coverage map to project-relative `documentation/tests.md` and the full report to `reports/test_plan_{timestamp}.md`.
