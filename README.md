...
**总协同 `/cmm-pm`** 站在所有阶段之上统筹整条流水线：管项目状态台账（跨会话不丢）、按阶段守门（风险结论→你拍板）、领航下一步该敲哪个命令。下面是它统筹的各阶段——按产品研发实际顺序排列，前面是决定"做不做、怎么做"的关键阶段，后面是落地与迭代。
...
本仓库是 Claude Code 多插件 marketplace。一条命令装全部 10 个插件：
...
```text
它会先和你确认核心信息（产品/人群/市场/盈利），再做数据驱动的市场分析，
结尾给出风险结论并问你是否进入下一步。逐阶段往下走即可。
```
